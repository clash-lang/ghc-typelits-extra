...
normaliseNat defs (TyConApp tc [x,y])
  | tc == typeNatAddTyCon = mergeNormResWith (\x' y' -> return (mergePlus x' y'))
                                            (normaliseNat defs x)
                                            (normaliseNat defs y)
  | tc == typeNatSubTyCon = mergeNormResWith (\x' y' -> return (mergeSub x' y'))
                                           (normaliseNat defs x)
                                           (normaliseNat defs y)
  | tc == typeNatMulTyCon = mergeNormResWith (\x' y' -> return (mergeMul x' y'))
                                           (normaliseNat defs x)
                                           (normaliseNat defs y)
